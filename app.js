...
function agentOptions(){const arr=[{name:val("leadName"),rank:val("leadRank")},...state.helpers].filter(x=>x.name);return arr}
...
$$("[data-next]").forEach(b=>b.onclick=()=>{
  ...
  if(next==="part2"){
    collectHelpers();
    if(!required(["station","stationAddress","actDate","actTime","leadRank","leadName","leadOni","detaineeName","detaineeDui","birthDate","civilStatus","occupation","detaineeAddress","detaineeMunicipality","detaineeDepartment","physicalFeatures","clothing","motherName","fatherName","parentsAddress","crimeName","crimeArticle","crimeLaw"]))return;
  ...
  if(next==="part4"){
    if(!required(["rightsText","objectsMode","detaineeSigns"]))return;
...
function buildAct(){
  ...
  $("acta").innerHTML=`
  <p class="heading">EN EL INTERIOR DE LA ${esc(val("station"))}, ${esc(val("stationAddress"))}; A LAS ${timeWords(val("actTime")).toUpperCase()} DEL DÍA ${dateWords(val("actDate")).toUpperCase()}. PRESENTE EL SUSCRITO ${esc(val("leadRank").toUpperCase())} ${keyPerson(val("leadName"))}, CON ORDEN NUMÉRICO INSTITUCIONAL ${digitsWords(val("leadOni")).toUpperCase()}${helperText.toUpperCase()}, DE GENERALES CONOCIDAS POR LABORAR TEMPORALMENTE EN LA ${esc(val("station").toUpperCase())}, DEJAMOS CONSTANCIA DE LA APREHENSIÓN EN FLAGRANCIA DEL SEÑOR ${keyPerson(val("detaineeName"))}, DE ${numberWords(val("age"))} AÑOS DE EDAD, ${esc(val("civilStatus"))}, ${esc(val("occupation"))}, RESIDENTE EN ${esc(val("detaineeAddress"))}, ${esc(val("detaineeMunicipality"))}, DEPARTAMENTO DE ${esc(val("detaineeDepartment"))}, QUIEN AL MOMENTO DE SU APREHENSIÓN SE IDENTIFICÓ CON SU DOCUMENTO ÚNICO DE IDENTIDAD NÚMERO ${digitsWords(val("detaineeDui"))}, DE LAS CARACTERÍSTICAS FÍSICAS SIGUIENTES: ${esc(val("physicalFeatures"))}; QUIEN VESTÍA DE LA SIGUIENTE MANERA: ${esc(val("clothing"))}; SIENDO HIJO DE LA SEÑORA ${keyPerson(val("motherName"))}${val("motherAge")?`, de ${numberWords(val("motherAge"))} años de edad`:""} Y DEL SEÑOR ${keyPerson(val("fatherName"))}${val("fatherAge")?`, de ${numberWords(val("fatherAge"))} años de edad`:""}, AMBOS RESIDENTES EN ${esc(val("parentsAddress"))}. DEJANDO CONSTANCIA DE LA APREHENSIÓN EN FLAGRANCIA DEL SEÑOR ${keyPerson(val("detaineeName"))}, POR EL DELITO DE ${esc(val("crimeName").toUpperCase())}, SEGÚN LO ESTABLECIDO EN EL ARTÍCULO ${numberWords(val("crimeArticle"))} DE ${esc(val("crimeLaw").toUpperCase())}.</p>

  <div class="section-title">RELATO DE LOS HECHOS</div>
  <p>${esc(val("story")).replaceAll("\n","<br>")}</p>
  ...
  <p>${esc(val("rightsText")).replaceAll("\n","<br>")}</p>

  <p>En cuanto a objetos relacionados con el procedimiento, ${objectText}. Y no habiendo nada más que hacer constar en la presente acta de aprehensión, se da por terminada la cual para mayor constancia firmamos, ${signText}.</p>

  ${annexText?`<p>${annexText}</p>`:""}
...
function draftData(){
  collectHelpers();collectAnnexes();collectCaptors();
  const ids=["station","stationAddress","actDate","actTime","leadRank","leadName","leadOni","detaineeName","detaineeDui","birthDate","age","civilStatus","occupation","detaineeAddress","detaineeMunicipality","detaineeDepartment","physicalFeatures","clothing","motherName","motherAge","fatherName","fatherAge","parentsAddress","crimeName","crimeArticle","crimeLaw","story","arrestDate","arrestTime","arrestPlace","arrestHamlet","arrestCanton","arrestMunicipality","arrestDepartment","rightsText","objectsMode","depositObjects","seizedObjects","detaineeSigns","noSignReason"];
  return {fields:Object.fromEntries(ids.map(id=>[id,val(id)])),helpers:state.helpers,annexes:state.annexes,captors:state.captors}
}
$("saveDraft").onclick=()=>{localStorage.setItem("siga_sv_v21_draft",JSON.stringify(draftData()));alert("Borrador guardado en este dispositivo.")};
function loadDraft(open=false){
  try{
    const d=JSON.parse(localStorage.getItem("siga_sv_v21_draft")||"null");if(!d){if(open)alert("No hay borrador guardado.");return}
