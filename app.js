...
function addCaptor(v="CAPTOR"){const i=state.captors.length;state.captors.push(v||"CAPTOR");const d=document.createElement("div");d.className="repeat-card";d.dataset.idx=i;d.innerHTML=`<label>Captor ${i+1} <small>(referencia interna)</small><select data-captor><option value="CAPTOR" ${v==="CAPTOR"?"selected":""}>CAPTOR</option><option value="CAPTOR PNC" ${v==="CAPTOR PNC"?"selected":""}>CAPTOR PNC</option><option value="CAPTOR FAES" ${v==="CAPTOR FAES"?"selected":""}>CAPTOR FAES</option></select></label>`;$("captors").appendChild(d)}
function renderCaptors(){const n=Number(val("captorCount")||state.captors.length||2);const old=[...state.captors];state.captors=[];$("captors").innerHTML="";for(let i=0;i<n;i++)addCaptor(old[i]||"CAPTOR")}
function collectCaptors(){state.captors=$$("#captors [data-captor]").map(x=>x.value)}
...
if($("captorCount"))$("captorCount").onchange=renderCaptors;
...
// Registro nominal de captores retirado de las firmas: las firmas usan solo etiquetas.
...
function storyNumberWords(text){
  let t=String(text||"");
  // Convierte números corrientes de 0 a 9999; conserva identificadores con guion, slash, puntos o números largos.
  return t.replace(/(^|[^\w\-\/.])(\d{1,4})(?![\w\-\/.])/g,(m,p,n)=>p+numberWords(n));
}
function buildAct(){
  ...
  let victimText="";
  if(val("victimMode")==="person") victimText=` En perjuicio de ${esc(val("victimTitle")||"la señora")} ${keyPerson(val("victimName"))}${val("victimAge")?` de ${numberWords(val("victimAge"))} años de edad`:""}${val("victimAddress")?`, residente en ${esc(val("victimAddress"))}`:""}${val("victimPhone")?`, quien puede ser localizada/o al número telefónico ${digitsWords(val("victimPhone"))}`:""},`;
  ...
  const detCount=Number(val("detaineeSignatureCount")||1);
  const detSigs=Array.from({length:detCount},()=>`<div class="signature detainee-signature">APREHENDIDO</div>`);
  const capSigs=state.captors.map(c=>`<div class="signature captor-signature">${esc(c)}</div>`);
  const sigs=[...detSigs,...capSigs].join("");
  ...
  <p><span class="section-title">RELATO DE LOS HECHOS.</span> ${esc(storyNumberWords(val("story")))}</p>
...
$("undoEdit").onclick=()=>document.execCommand("undo",false,null);
$("redoEdit").onclick=()=>document.execCommand("redo",false,null);
...
function clearForNewAct(){
  localStorage.removeItem("siga_sv_v3_draft");
  document.querySelectorAll("#wizard input,#wizard textarea").forEach(el=>{if(el.type!=="button")el.value=""});
  document.querySelectorAll("#wizard select").forEach(el=>el.selectedIndex=0);
  state.helpers=[];state.annexes=[];state.captors=[];renderHelpers();renderAnnexes();
  if($("captorCount"))$("captorCount").value="2";if($("detaineeSignatureCount"))$("detaineeSignatureCount").value="1";renderCaptors();
  $("acta").innerHTML="";$("duiPreview").src="";$("duiCamera").value="";$("duiScanPanel").classList.add("hidden");
  const d=new Date(),iso=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;$("actDate").value=iso;
  showPart("part1");
}
$("finishAct").onclick=()=>{if(!confirm("¿Guardar el acta en Word y limpiar todos los datos para una nueva acta?"))return;$("exportWord").click();setTimeout(()=>{clearForNewAct();alert("Acta guardada. SIGA SV quedó limpia para una nueva acta.")},500)};

$("printAct").onclick=()=>window.print();

function draftData(){
  collectHelpers();collectAnnexes();collectCaptors();
  const ids=["station","stationAddress","actDate","actTime","leadRank","leadName","leadOni","detaineeName","aliasMode","alias","detaineeDui","birthDate","age","civilStatus","occupation","detaineeAddress","detaineeMunicipality","detaineeDepartment","physicalFeatures","clothing","motherName","motherAge","fatherName","fatherAge","parentsAddress","crimeName","crimeArticle","crimeLaw","victimMode","victimTitle","victimName","victimAge","victimAddress","victimPhone","publicInterest","victimCustom","detaineeSignatureCount","captorCount","story","arrestDate","arrestTime","arrestPlace","arrestHamlet","arrestCanton","arrestMunicipality","arrestDepartment","rightsText","objectsMode","depositObjects","seizedObjects","noticeMode","noticeNobodyReason","noticePersonName","noticeRelation","detaineeSigns","noSignReason","finalObservation"];
