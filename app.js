...
const state={helpers:[],annexes:[],captors:["CAPTOR","CAPTOR"]};
const CRIMES=[
...
function addCaptor(v="CAPTOR"){state.captors.push(v);renderCaptors()}
function renderCaptors(){
  const count=Number(val("captorCount")||state.captors.length||2);
  while(state.captors.length<count)state.captors.push("CAPTOR");
  state.captors=state.captors.slice(0,count);
  $("captors").innerHTML=state.captors.map((v,i)=>`<label>Captor ${i+1}<select data-captor-type data-i="${i}"><option ${v==="CAPTOR"?"selected":""}>CAPTOR</option><option ${v==="CAPTOR PNC"?"selected":""}>CAPTOR PNC</option><option ${v==="CAPTOR FAES"?"selected":""}>CAPTOR FAES</option></select></label>`).join("");
  $$('[data-captor-type]').forEach(x=>x.onchange=()=>state.captors[Number(x.dataset.i)]=x.value);
}
function collectCaptors(){state.captors=$$('[data-captor-type]').map(x=>x.value)}
$("newAct").onclick=()=>{showPart("part1");if(!state.helpers.length)addHelper();renderCaptors()};
...
$("victimMode").onchange=()=>{
  ...
  $("victimCustomFields").classList.toggle("hidden",val("victimMode")!=="custom");
};
$("noticeMode").onchange=()=>{
...
$$("[data-next]").forEach(b=>b.onclick=()=>{
  ...
  if(next==="part2"){
    ...
    if(val("victimMode")==="custom"&&!required(["victimCustom"]))return;
...
function buildAct(){
  ...
  let victimText="";
  if(val("victimMode")==="person") victimText=` En perjuicio de la señora ${keyPerson(val("victimName"))}${val("victimAge")?` de ${numberWords(val("victimAge"))} años de edad`:""}${val("victimAddress")?`, residente en ${esc(val("victimAddress"))}`:""}${val("victimPhone")?`, quien puede ser localizada al número telefónico ${esc(val("victimPhone"))}`:""},`;
  if(val("victimMode")==="public") victimText=` En perjuicio de ${esc(val("publicInterest"))},`;
  if(val("victimMode")==="custom") victimText=` En perjuicio de ${esc(val("victimCustom"))},`;
  let noticeText="";
  ...
  const detainees=Array.from({length:Number(val("detaineeSignatureCount")||1)},()=>`<div class="signature detainee-sign">APREHENDIDO</div>`).join("");
  const captorSigs=state.captors.map(c=>`<div class="signature captor-sign">${esc(c)}</div>`).join("");
  const sigs=`<div class="detainee-signatures">${detainees}</div><div class="captor-signatures">${captorSigs}</div>`;
...
$("captorCount").onchange=renderCaptors;
renderCaptors();

// Archivo local de actas terminadas: limpiar formulario no elimina las actas guardadas.
function savedActs(){try{return JSON.parse(localStorage.getItem("siga_sv_saved_acts")||"[]")}catch{return []}}
function renderSavedActs(){const acts=savedActs();$("savedCount").textContent=acts.length;$("savedActsList").innerHTML=acts.length?acts.map((a,i)=>`<div class="saved-item"><div><b>${esc(a.serial)}</b><small>${esc(a.savedAt)} · ${esc(a.detainee||"Sin nombre")}</small></div><span><button class="secondary mini" data-open-saved="${i}">ABRIR</button></span></div>`).join(""):`<p class="notice">Todavía no hay actas guardadas.</p>`;$$('[data-open-saved]').forEach(b=>b.onclick=()=>{const a=acts[Number(b.dataset.openSaved)];if(!a)return;localStorage.setItem("siga_sv_v3_draft",JSON.stringify(a.data));loadDraft(true)})}
$("openSavedActs").onclick=()=>{$("savedActsPanel").classList.remove("hidden");renderSavedActs()};
$("closeSavedActs").onclick=()=>$("savedActsPanel").classList.add("hidden");
function clearVariableForm(){localStorage.removeItem("siga_sv_v3_draft");$$('#wizard input, #wizard textarea').forEach(e=>{if(["station","stationAddress","rightsText"].includes(e.id))return;e.value=""});$$('#wizard select').forEach(e=>{if(["detaineeSignatureCount","captorCount"].includes(e.id))return;e.selectedIndex=0});state.helpers=[];state.annexes=[];state.captors=["CAPTOR","CAPTOR"];renderHelpers();renderAnnexes();renderCaptors();$("acta").innerHTML=""}
$("saveFinalAct").onclick=()=>{collectHelpers();collectAnnexes();collectCaptors();if(!buildAct())return;const acts=savedActs();const n=(Number(localStorage.getItem("siga_sv_serial")||"0")+1);localStorage.setItem("siga_sv_serial",String(n));const serial=`SIGA-${String(n).padStart(5,"0")}`;acts.unshift({serial,savedAt:new Date().toLocaleString("es-SV"),detainee:val("detaineeName"),data:draftData(),html:$("acta").innerHTML});localStorage.setItem("siga_sv_saved_acts",JSON.stringify(acts));alert(`Acta guardada ✓\nSerial: ${serial}`);clearVariableForm();renderSavedActs();showOnly("home")};
renderSavedActs();


function draftData(){
  collectHelpers();collectAnnexes();collectCaptors();
  const ids=["station","stationAddress","actDate","actTime","leadRank","leadName","leadOni","detaineeName","aliasMode","alias","detaineeDui","birthDate","age","civilStatus","occupation","detaineeAddress","detaineeMunicipality","detaineeDepartment","physicalFeatures","clothing","motherName","motherAge","fatherName","fatherAge","parentsAddress","crimeName","crimeArticle","crimeLaw","victimMode","victimName","victimAge","victimAddress","victimPhone","publicInterest","victimCustom","detaineeSignatureCount","captorCount","story","arrestDate","arrestTime","arrestPlace","arrestHamlet","arrestCanton","arrestMunicipality","arrestDepartment","rightsText","objectsMode","depositObjects","seizedObjects","noticeMode","noticeNobodyReason","noticePersonName","noticeRelation","detaineeSigns","noSignReason","finalObservation"];
